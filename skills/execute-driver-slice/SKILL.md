...
10. Commit using the activity's exact Conventional Commit subject and add exactly one `Plan-Activity: ID` trailer. Push the commit, run `check --strict --pushed`, and wait for the required GitHub Actions run. Do not start another activity until both the pushed check and CI pass.

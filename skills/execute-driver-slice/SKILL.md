...
3. Start only that activity with `start ID`. For additional tasks, reassert the user-authorized declared track with `start ID --track TRACK`; it must match the selected activity, and the same track remains in force for the goal. Do not edit submodules, reference pins, or unrelated files.
...
9. Run `complete ID`, `render-state`, and `check`. Inspect the diff for one coherent unit.

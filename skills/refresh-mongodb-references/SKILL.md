...
5. Review every reported commit, changed path, mapped landmark, and affected activity. For specifications, also review every added, removed, or changed accepted document, fixture file, conformance case, and unified test. Inspect the upstream patches before deciding that a changed fingerprint has no Lua impact.
6. Do not accept a report whose generator simulation is not repeatable. For each completed or active activity affected by semantic drift, run `python3 planning/update_plan.py review ID --reason "..."`. Add the smallest pending activities needed for new behavior and assign new classifications to concrete owners.

...
`update_references.py --dry-run` reports the candidate commits, changed paths and landmarks, and affected roadmap activities without moving the checkout or pin. Without `--dry-run`, the command advances one pin, rejects dirty or drifted checkouts, checks commit ancestry and mapped landmarks, and regenerates the affected artifacts. It leaves the resulting changes unstaged for review.

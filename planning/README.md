...
python3 spec/v102/scope.py [--check]
...
`update_readme_compatibility.py` projects the conformance ledger and selected accepted-specification prose requirements into the README's compatibility table. It groups suites by the [MongoDB driver specifications onion model](https://alexbevi.com/specifications/). Machine-backed authentication and GridFS requirements are combined with their fixture cases, while prose-only suites use catalog evidence alone.

A suite is complete when every scored outcome passes, partial when passing and incomplete outcomes coexist, and not implemented when no scored outcome passes. Terminal unsupported suites use the white will-not-implement badge. The passing percentage excludes `not_applicable`, `no_machine_cases`, terminal `unsupported`, explicit unscored exclusions, and fixtures restricted to MongoDB versions older than the supported 7.0 floor. A dagger marks rows with excluded fixtures, and the generated note below the table gives their counts. The total counts all scored outcomes instead of averaging suite percentages. Run the generator whenever ledger or projected catalog statuses change. CI uses `--check` to catch drift.

`spec/conformance/catalog.py` inventories every document marked Accepted in the pinned specifications checkout, fingerprints its exact bytes, and assigns its suite to one explicit onion-model layer. Unlike the fixture ledger, the catalog includes prose-only specifications and test plans. Every prose-only document, plus explicitly classified normative requirements from machine-backed specifications such as GridFS, has a stable record with a roadmap owner, scope, status, runner, and exact execution evidence or reason. `not_applicable` identifies a language or API mismatch, `no_machine_cases` identifies normative prose with no portable executable assertions, and `unsupported` records a terminal project capability decision without claiming execution. None enters the pass rate. The checked artifact fails when an accepted document changes or a suite or prose requirement is added, removed, or left unclassified.
...
`spec/v10/scope.py` projects all 40 dedicated load-balancer cases and every unified identity with a load-balanced runOn branch. It requires exact runners and completed owners for the 39 executable dedicated cases, preserves the one normative upstream skip, and requires exact Full Conformance passes for all 738 runnable cross-suite identities. The 235 encryption branches join nine backpressure branches and the explicit OCSP and SOCKS5 terminal unsupported decisions. `make test-v10-scope` validates the committed projection; Linux and requested macOS Full Conformance supply the authoritative aggregate execution reports.

`spec/v102/scope.py` projects eleven pinned GSSAPI configuration cases and ten explicit normative requirements for credential normalization, hostname handling, the SASL conversation, the packaged provider, standalone credential modes, replica-set discovery, and concurrent contexts. Every record must pass with its exact runner, environment, and completed implementation owner. The closure owner may be active or complete. Any stale `AUTH-019` deferral, missing runner, unverified platform claim, or ratchet reduction fails `make test-v102-scope`. The provider matrix claims only the recurring Ubuntu 24.04 and Lua 5.4 live profile.

`spec/v103/scope.py` projects four passing shared logging requirements and the exact 230 standardized observability cases assigned to command, selection, SDAM, and CMAP logging slices plus the terminal client-backpressure and OpenTelemetry decisions. It rejects residual `ADV-009` fixture or capability ownership, mismatched unified capability owners, missing deterministic foundation runners, changed owner counts, and ratchet reductions. `make test-v103-scope` validates the committed projection; Full Conformance checks the same artifact on Linux and macOS without claiming component emission. The prose-only Atlas Secure Frontend Processor test profile is likewise terminal unsupported and remains visible as an unscored compatibility row without an external-environment execution claim.
...
The unified specification runner is test-only support under `spec/support/mongodb/unified`. `spec/module-classification.json` classifies every packaged module and top-level export, along with each test-only `mongodb.unified.*` module; package and coverage contract tests reject missing classifications, test-only modules in the rockspec, and test-only modules in the production coverage baseline. Use the Make targets so `spec/support/?.lua` and `spec/support/?/init.lua` are added explicitly to the Lua test search path.

...
`update_readme_compatibility.py` projects the conformance ledger and selected prose-only accepted-specification catalog requirements into the README's driver-layer compatibility table. Its tracked-support percentage counts passed outcomes among all support-scored outcomes and excludes `not_applicable` and `no_machine_cases`. Run it whenever ledger or projected catalog statuses change; CI uses `--check` so the README cannot drift from accountable evidence.

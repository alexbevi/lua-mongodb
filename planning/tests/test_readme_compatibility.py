...
class ReadmeCompatibilityTests(unittest.TestCase):
  ...
  def test_unscored_fixtures_are_disclosed(self) -> None:
    ...
    self.assertIn("|  | **Total** |  | **99.9%** **†** |", table)
  ...
  def test_table_follows_the_onion_and_covers_every_suite(self) -> None:
    ...
    self.assertIn("|  | **Total** |  | **99.9%** **†** |", table)
    self.assertIn(
      "| Programmability | [Client-side encryption](https://alexbevi.com/"
      "specifications/client-side-encryption/client-side-encryption.html) "
      "| ⚪ | N/A **†** |",
      table,
    )
    self.assertIn(

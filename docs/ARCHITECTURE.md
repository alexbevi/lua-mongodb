...
Every case records the exact specification commit, source suite and format, content fingerprint, runner or explicitly pending runner, required environment, milestone scope, status, last execution command, and roadmap activity. Passing entries must name an implemented runner and evidence; unsupported entries must be owned by an incomplete activity. Generation fails on an unknown suite, and validation fails on added, removed, or changed files or cases, malformed records, missing runners or owners, and deferrals assigned to completed activities. The current ledger covers 2,966 files and 5,524 cases across 31 suites, with 2,063 passing and 3,461 explicitly deferred cases; standard unified inventory verification runs its unit contract and stale-generation check.

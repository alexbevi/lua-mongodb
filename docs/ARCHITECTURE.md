...
All 1,900 tests in the 483 discovered integration fixtures are classified to the driver slice that can first execute them; client-side encryption is explicitly outside the roadmap pending a separate design. `spec/unified/executors.json` names every exact runnable identity and its required environment. It currently registers the first `insertOne` case and 45 standalone collection-read cases, raising both passing and runnable ratchets to 46. Database-level aggregation remains assigned to release conformance because it is not part of the v1 public collection adapter, and the six pre-4.4 unacknowledged-hint cases remain explicit compatibility deferrals because v1 targets MongoDB 7.0–8.2. The generator's `--check` mode prevents hand-edited or stale classification drift, while normal reports retain a row for every selected test so exclusions cannot disappear into aggregate counts.

`mongodb.unified.driver` connects the lifecycle's internal client and client, database, and collection entity factories to the public Lua driver. Its initial-data adapter uses majority writes, while its outcome adapter uses primary/local reads sorted by `_id`. Collection operation descriptors translate only declared unified arguments for `insertOne`, `find`, `aggregate`, `countDocuments`, `estimatedDocumentCount`, `distinct`, and the three `findOneAnd*` operations. Numeric wrappers and return-document enums are converted at this boundary; iterable results are coerced to BSON arrays, and cursors are closed on exhaustion or error before matching. Runtime and URI remain injected.

`spec/unified/execute.lua` rejects every identity absent from the exact executor registry. The first insert case retains its deterministic LuaSocket/Copas loopback service. Collection reads use the public driver against a real standalone so query, collation, aggregation, array-filter, and find-and-modify semantics are not reimplemented in the harness. The Python execution envelope starts and stops an ephemeral `mongod`, or uses an explicitly supplied URI and server version. CI provisions MongoDB 8.0 on Linux and macOS. The report currently records 46 executions and passes, zero failures or environment skips, and 1,854 explicit deferrals.

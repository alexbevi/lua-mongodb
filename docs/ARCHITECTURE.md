...
All 1,900 tests in the 483 discovered integration fixtures are classified to the driver slice that can first execute them; client-side encryption is explicitly outside the roadmap pending a separate design. The first `crud/tests/unified/insertOne.json::test[1]` case is runnable and raises the passing/runnable ratchets to one. Explicit per-test overrides identify deliberately selected first cases without weakening the general classification rules. Later activities change only their newly supported manifest entries to `runnable` in the same commit as the implementation and tests. The generator's `--check` mode prevents hand-edited or stale classification drift, while normal reports retain a row for every selected test so exclusions cannot disappear into aggregate counts.

`mongodb.unified.driver` connects the lifecycle's internal client and client, database, and collection entity factories to the public Lua driver. Its initial-data adapter uses majority writes; its outcome adapter uses primary/local reads sorted by `_id`; and its first operation descriptor accepts only `insertOne.document` and converts the public result to unified `insertedId`. Runtime and URI are injected. `spec/unified/execute.lua` registers only the selected pinned identity and supplies a deterministic LuaSocket/Copas MongoDB loopback service; an unregistered runnable identity exits as a failure. The normal execution report therefore records one actual execution and pass while retaining 1,899 explicit deferrals, rather than presenting classification as execution.
...
Every case records the exact specification commit, source suite and format, content fingerprint, runner or explicitly pending runner, required environment, milestone scope, status, last execution command, and roadmap activity. Passing entries must name an implemented runner and evidence; unsupported entries must be owned by an incomplete activity. Generation fails on an unknown suite, and validation fails on added, removed, or changed files or cases, malformed records, missing runners or owners, and deferrals assigned to completed activities. The current ledger covers 2,966 files and 5,524 cases across 31 suites; standard unified inventory verification runs its unit contract and stale-generation check.

...
describe("BSON primitive codec", function()
  it("round trips ordered documents and unambiguous primitive values", function()
    ...
    assert.are.equal(42, array:get(4))
    assert.are.equal(2147483648, array:get(5))
    assert.are.equal(1.25, array:get(6))
  ...
  it("preserves the full signed Lua integer range", function()
    ...
    assert.are.equal(-2147483648, decoded:get("int32_min"))
    assert.are.equal(-2147483649, decoded:get("int64_negative"))
    assert.are.equal(math.mininteger, decoded:get("int64_min"))
    assert.are.equal(math.maxinteger, decoded:get("int64_max"))
  end)
end)

...
def discover_fixtures(source: Path, includes: list[str] | None = None) -> list[str]:
  ...
  for path in source.rglob("*.json"):
    ...
    is_collection_bulk_monitoring_fixture = relative.as_posix() == (
      "command-logging-and-monitoring/tests/monitoring/bulkWrite.json"
    )
    ...
    is_insert_many_monitoring_fixture = relative.as_posix() == (
      "command-logging-and-monitoring/tests/monitoring/insertMany.json"
    )
    ...
    if not (
      ...
      or is_collection_bulk_monitoring_fixture
      ...
      or is_connection_logging_fixture

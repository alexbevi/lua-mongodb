...
class PublishTests(unittest.TestCase):
  def test_release_metadata_is_derived_from_the_rockspec(self) -> None:
    metadata = publish.release_metadata(ROOT / "mongodb-0.10.0-1.rockspec")

    self.assertEqual("mongodb", metadata.package)
    self.assertEqual("0.10.0", metadata.version)
    self.assertEqual("0.10.0-1", metadata.rockspec_version)
    self.assertEqual("v0.10.0", metadata.tag)
    self.assertEqual("mongodb-0.10.0-1.src.rock", metadata.source_rock)

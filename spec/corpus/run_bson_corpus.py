#!/usr/bin/env python3
"""Run the pinned BSON corpus against the pure-Lua codec."""
...
def lua_program(
  valid: list[dict[str, Any]],
  decode_errors: list[dict[str, Any]],
  decimal_errors: list[dict[str, Any]],
) -> str:
  valid_rows = []

  for case in valid:
    degenerate = case.get("degenerate_bson")
    valid_rows.append("{%s,%s,%s,%s}" % (
  ...
  decimal_rows = [lua_string(case["string"]) for case in decimal_errors]
...
for index, vector in ipairs(valid) do
  ...
  if vector[3] then
    ...
    assert(
      assert(bson.encode(degenerate)) == from_hex(vector[4]),
...
for index, input in ipairs(decimal_errors) do
  assert(not pcall(bson.decimal128, input), "parse-error index " .. index .. " accepted")
end

print(#valid .. " canonical BSON vectors passed")
print(#decode_errors .. " BSON decode-error vectors passed")
print(#decimal_errors .. " Decimal128 parse-error vectors passed")
'''
...
def main() -> int:
  ...
  decimal_errors = [case for case in parse_errors if case["suite"].startswith("decimal128-")]
  deferred_parse = len(parse_errors) - len(decimal_errors)
  process = subprocess.run(
    [arguments.lua, "-"],
    cwd=ROOT,
    env=os.environ.copy(),
    input=lua_program(valid, decode_errors, decimal_errors),
    text=True,
    capture_output=True,
  )
  ...
  if process.returncode != 0:
    ...
    print(explain_failure(output, valid, decode_errors, decimal_errors), file=sys.stderr)
  ...
  print(
    f"deferred until UTF-001: {extended_json_count} Extended JSON representations "
    f"and {deferred_parse} non-Decimal parse-error vectors"
  )
  return 0

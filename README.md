...
`spec/release/scope.json` overlays the complete 5,524-case conformance ledger with the reviewed production-core boundary. It rejects ambiguous final-checkpoint owners and distinguishes 246 applicable gaps assigned to concrete release slices from 1,900 intentional post-v1 exclusions assigned to named capabilities. Regenerate it only with the capability and conformance ledgers; `make test-conformance` rejects stale scope counts, completed gap owners, exclusions without a roadmap rationale, and any case assigned directly to the final release checkpoint.

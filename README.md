...
This repository is the planning and implementation workspace for a pure-Lua MongoDB driver. It is pre-alpha: standalone and monitored replica-set clients, `run_command`, core CRUD, collection bulk-write, administration, multi-batch cursors, causal sessions, and retryable reads are implemented while production execution behavior is added incrementally.
...
Every target checks its prerequisites and explains how to select a missing tool through `LUA`, `LUAROCKS`, `BUSTED`, `LUACHECK`, or `PYTHON`. The unified execution gate also needs `mongod` on `PATH` or selected through `MONGOD`; callers may instead supply a dedicated disposable standalone through `MONGODB_UNIFIED_URI` and `MONGODB_UNIFIED_SERVER_VERSION`. External deployments set `MONGODB_UNIFIED_TEST_COMMANDS=1` when test commands are enabled; registered failpoint cases are otherwise explicitly reported as environment-skipped. The ephemeral gate enables test commands itself. `make test-unit` includes every pinned BSON and Extended JSON corpus representation, all 98 non-SRV connection-string fixtures plus their option-warning semantics, and the deterministic unified runner core. `make test-unified` separately runs all 320 JSON meta-fixtures, parses and schema-validates all 483 pinned integration files through the Lua Extended JSON and schema implementations, inventories their 1,900 tests, and reports execution status. The gate executes 556 exact cases, including 243 retryable-read cases covering retry eligibility, error codes, monitoring, implicit sessions, aggregate write stages, and database, collection, and index enumeration; the other 1,344 cases retain explicit owners. Integration coverage also includes real Copas/LuaSocket transport, verified and insecure LuaSec handshakes, public standalone and replica-set lifecycles, OP_MSG handshake, SCRAM-SHA-256 authentication, authenticated command execution, and reconnecting retry attempts.
...
`make test-conformance` checks the generated normative coverage ledger at `spec/conformance/ledger.json`. The ledger pins 2,966 JSON/YAML source files and 5,524 stable cases across 31 specification suites, including unified and legacy BSON, URI, SDAM, selection, CMAP, sessions, and timeout formats; 2,556 cases currently have passing evidence. Each case records its fingerprint, format, runner, environment, milestone scope, status, last execution command, and roadmap owner. JSON is the canonical executable form where upstream also supplies equivalent YAML; both source files remain fingerprinted. Regenerate deliberately with `python3 spec/conformance/ledger.py`, while the normal gate uses `--check` so additions, removals, edits, missing owners, and stale coverage fail.

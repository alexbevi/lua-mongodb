...
local function operation_error(operation, category)
  return errors.new({
    category = category,
    message = "Zstandard " .. operation .. " failed",
  })
end

local function transform(binding, method, operation, data, category)
  require_string("Zstandard " .. operation .. " input", data)
...
function M.new(binding)
  if type(binding) ~= "table"
    ...
    error("Zstandard binding must provide compress and decompress functions", 2)
  end
...
function M.load(loader)
  ...
  if type(loader) ~= "function" then
    error("Zstandard loader must be a function", 2)
  end

  local outcome = table.pack(pcall(loader, "zstd"))
